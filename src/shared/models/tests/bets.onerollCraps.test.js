...
test ("One roll > Any Craps > Free chips decreased by bet amount", () => {
    // Setup
    ...
    const betType = {
        value: betAmount,
        typePath: "$.onerollCraps.baseBet",
        action: "add"
    };

    bets.addBet(appState, betType);

    expect( bets.getBets().onerollCraps.baseBet ).toBe( betAmount );
    expect( bets.getFreeChips() ).toBe( Constants.STARTING_CHIP_AMOUNT - betAmount );

});

test ("One roll > Any Craps > Lose combo 1", () => {
    // Setup
    const appState = new AppController();
    const bets = new Bets();
    ...
    expect( bets.getBets().onerollCraps.baseBet ).toBe( 0 );
    expect( bets.getFreeChips() ).toBe( Constants.STARTING_CHIP_AMOUNT - betAmount );
    expect( bets.getAllChips() ).toBe( Constants.STARTING_CHIP_AMOUNT - betAmount );

});

test ("One roll > Any Craps > Lose combo 2", () => {
    // Setup
    const appState = new AppController();
    const bets = new Bets();
...
test ("One roll > Any Craps > Correct payout on a win", () => {
    // Setup
    const appState = new AppController();
    const bets = new Bets();
    ...
    expect( bets.getBets().onerollCraps.baseBet ).toBe( 0 );
    expect( bets.getAllChips() ).toBe( Constants.STARTING_CHIP_AMOUNT + (betAmount * Constants.PAYOUT_ONETIME_ANYCRAPS) );
});